...
Table~\ref{tab:representation_results} tests whether the 16-dimensional latent representation retains the discriminative information in the 69 normalized features. Under S2, the original features increased macro recall from 0.5620 to 0.6483 and macro F1 from 0.4249 to 0.5406. The latent representation reduced dimensionality by 76.81\% and shortened S2 classifier training by 49.7\%, but this timing excludes Autoencoder training. Prior studies report favorable Autoencoder-based reduction, including more than 80\% fewer transmitted features with little detection loss and strong Random Forest--Autoencoder performance \cite{HariVinayak2025,Li2024FeatureReduction}. The controlled baseline here shows that those gains do not transfer automatically across datasets and classifiers. H2 is therefore partially supported: the latent features remain useful and compact, but they do not preserve all information needed for classification and should not be claimed to outperform the original features.
